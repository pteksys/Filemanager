...
Component {
    ...
    AbstractButton {
        id: button
        property bool smallText: action.hasOwnProperty("smallText")
        property bool showText: action.hasOwnProperty("showText")
        action: modelData
        ...
        Row {
            ...
            Label {
                ...
                visible: text !== "More" && !button.showText
                width: visible ? paintedWidth : 0
                textSize: button.smallText ? Label.Small : Label.Medium
                color: {
                    if (button.enabled)
                        return text === i18n.tr("Pick") ? theme.palette.selected.backgroundText : theme.palette.normal.backgroundText
